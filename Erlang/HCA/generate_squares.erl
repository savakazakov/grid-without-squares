...
-export([get_squares/1, get_heat_map/1, get_dependency_map/2, convertToBitlist/2, convertAllToBitlist/2]).
...
% This integer will, when expressed as a binary, be entirely cleared bits, 
...
% Where the values in each cell of the grid refer to the number of times that cell is a corner to a square.
% This can be used to determine the cells that are most used & hence would be most efficiently set to 0.

% These squares can also be combined together via bitwise OR
% If this all of the squares that contain a 1 in a cell are OR'd together a dependency_map is created
...
get_heat_map([Square |[]], Map) -> elementwise_add_list(Square, Map);
get_heat_map([Square | Squares], Map) -> get_heat_map(Squares, elementwise_add_list(Square, Map)).
...
combine_squares(Squares, GridSize) -> lists:foldl(fun elementwise_bor_list/2, lists:duplicate(GridSize, 0), Squares). % Bitwise OR all of the squares together
...
elementwise_bor_list(List1, List2) -> [X bor Y || {X, Y} <- lists:zip(List1, List2)].
elementwise_add_list(List1, List2) -> [X + Y || {X, Y} <- lists:zip(List1, List2)].
